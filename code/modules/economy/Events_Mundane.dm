	...
	if(!body)
		...
		switch(event_type)
			...
			if(MOVIE_RELEASE)
				...
				[pick("Deadly","The last","Lost","Dead")] [pick("Starships","Warriors","outcasts","Etharians",SPECIES_UNATHI,SPECIES_SKRELL)] \
...
/datum/event/trivial_news/announce()
	...
	var/body = pick(
	"Tree stuck in etharian; firefighters baffled.",\
	...
	"'Here kitty kitty' no longer preferred etharian retrieval technique.",\
	...
	"Etharian Burglar Spotted on [affected_dest.name], Mistaken For Dalmatian",\
	...
	"Boy Saves Etharian From Tree on [affected_dest.name], Thousands Cheer",\
	"Shipment Of Apples Overturns, [affected_dest.name] Diner Offers Applesauce Special",\
	"Spotted Owl Spotted on [affected_dest.name]",\
	"Humans Everywhere Agree: Purring Etharians Are Happy Etharians",\
